...
<div id="selectorWrapper" runat="server">
    <sf:FlatSelector ID="itemsSelector" runat="server" ItemType="Telerik.Sitefinity.Ecommerce.Catalog.Model.Product"
		...
		ServiceUrl="~/Sitefinity/Services/Ecommerce/Catalog/ProductService.svc">
        <DataMembers>

            <sf:DataMemberInfo runat="server" Name="Title" IsExtendedSearchField="true" HeaderText='Title'>
                 <img sys:src="{{Item.Thumbnail.Url}}" width="50" />
                <strong>{{Item.Title.Value}}</strong>
               
            </sf:DataMemberInfo>
            <sf:DataMemberInfo runat="server" Name="DisplayPriceFormatted" HeaderText='Price'>
                <span>{{Item.DisplayPriceFormatted}}</span>
            </sf:DataMemberInfo>
        </DataMembers>
    </sf:FlatSelector>
...
<ul id="selectedItemsList" data-template="ul-template" data-bind="source: items" runat="server" class="sfCategoriesList"></ul>
<script id="ul-template" type="text/x-kendo-template">
    <li>
        <span data-bind="text: Item.Title.Value"> </span>
